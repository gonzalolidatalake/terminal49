-- Add optional ETD date-range filter parameters to get_container_tracking_summary.
--
-- New parameters:
--   p_etd_from DATE DEFAULT NULL
--   p_etd_to   DATE DEFAULT NULL
--
-- Filter uses COALESCE(s.pol_etd_at, s.pol_atd_at) so that containers with a
-- known actual departure but no estimated departure are still matched.
--
-- Drop the previous 7-parameter overload first to avoid ambiguity (42725).
DROP FUNCTION IF EXISTS get_container_tracking_summary(TEXT, TEXT, VARCHAR, VARCHAR, VARCHAR, INT, INT);
...
BEGIN
    ...
    WHERE
        ...
        AND (p_etd_from   IS NULL OR COALESCE(s.pol_etd_at, s.pol_atd_at)::date >= p_etd_from)
        AND (p_etd_to     IS NULL OR COALESCE(s.pol_etd_at, s.pol_atd_at)::date <= p_etd_to)
...
COMMENT ON FUNCTION get_container_tracking_summary IS
    ...
    'Supports optional ETD date-range filtering via p_etd_from / p_etd_to (inclusive), '
    'falling back to ATD when ETD is null.';
