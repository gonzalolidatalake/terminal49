-- Fix type mismatch on transshipment_locodes / transshipment_names columns.
--
-- RETURNS TABLE declares both as text[], but array_agg over character varying
-- columns produces character varying[], triggering PostgreSQL error 42804
-- ("column N of relation has type text[] but expression is of type character varying[]").
--
-- Fix: explicit ::text[] casts on both array_agg expressions in the SELECT body.
