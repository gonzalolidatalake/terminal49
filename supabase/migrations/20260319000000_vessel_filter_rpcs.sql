-- Vessel filter additions — v1.7.0
--
-- Changes:
--   1. get_container_tracking_summary — adds p_vessel_filter text DEFAULT NULL
--      Filters on shipments.vessel_name (main voyage vessel, not last_vessel_name).
--
--   2. get_shipment_kpis — adds p_vessel_filter text DEFAULT NULL
--      Same filter applied to filtered_shipments CTE.
--
--   3. get_shipment_filter_options — adds vessel_names jsonb output column.
--      Return type change requires DROP before CREATE OR REPLACE.
--
-- All other parameters, logic, and output columns are preserved exactly.

-- ============================================================================
-- RPC 1 — get_container_tracking_summary (add p_vessel_filter)
-- ============================================================================

-- NOTE: Adding a new parameter with DEFAULT to an existing function is allowed
-- by CREATE OR REPLACE as long as the return type is unchanged.
-- The return type here is identical to v1.6.0 (total_shipment_count included).

DROP FUNCTION IF EXISTS get_container_tracking_summary(text,text,text,text,text,date,date,integer,integer);
...
COMMENT ON FUNCTION get_container_tracking_summary IS
    ...
    'to ATD when ETD is null), and vessel filter (p_vessel_filter, matches shipments.vessel_name). '
...
-- ============================================================================
-- RPC 2 — get_shipment_kpis (add p_vessel_filter)
-- ============================================================================

CREATE OR REPLACE FUNCTION get_shipment_kpis(
    p_search         text DEFAULT NULL,
    p_status         text DEFAULT NULL,
    p_scac           text DEFAULT NULL,
    p_pol_locode     text DEFAULT NULL,
...
COMMENT ON FUNCTION get_shipment_kpis IS
    ...
    'p_vessel_filter matches shipments.vessel_name (main voyage vessel).';

-- ============================================================================
-- RPC 3 — get_shipment_filter_options (add vessel_names jsonb output column)
-- ============================================================================

-- Return type change requires DROP before CREATE OR REPLACE.
DROP FUNCTION IF EXISTS get_shipment_filter_options();

CREATE FUNCTION get_shipment_filter_options()
RETURNS TABLE (
    statuses       text[],
    scacs          text[],
    pol_locodes    text[],
...
BEGIN
    ...
    pol AS (
        SELECT
            port_of_lading_locode                                      AS locode,
            MIN(raw_data -> 'attributes' ->> 'port_of_lading_name')    AS name
        FROM shipments
        WHERE port_of_lading_locode IS NOT NULL
        GROUP BY port_of_lading_locode
        ORDER BY port_of_lading_locode
    ),
    pod AS (
        SELECT
            port_of_discharge_locode                                       AS locode,
            MIN(raw_data -> 'attributes' ->> 'port_of_discharge_name')     AS name
        FROM shipments
        WHERE port_of_discharge_locode IS NOT NULL
        GROUP BY port_of_discharge_locode
        ORDER BY port_of_discharge_locode
    )
    SELECT
        -- Container statuses
        ARRAY(
        ...
        -- Shipping line SCACs
        ARRAY(
            SELECT DISTINCT shipping_line_scac
            FROM shipments
            WHERE shipping_line_scac IS NOT NULL
            ORDER BY 1
        )::text[],
        -- POL: parallel arrays (locodes[i] ↔ names[i] describe the same port)
        ARRAY(SELECT locode FROM pol)::text[],
        ARRAY(SELECT name   FROM pol)::text[],
        -- POD: parallel arrays
        ARRAY(SELECT locode FROM pod)::text[],
        ARRAY(SELECT name   FROM pod)::text[],
        -- Vessel names (sorted, nulls excluded)
        (SELECT jsonb_agg(DISTINCT s.pod_vessel_name ORDER BY s.pod_vessel_name)
                FILTER (WHERE s.pod_vessel_name IS NOT NULL)
         FROM shipments s);
END;
$$;

COMMENT ON FUNCTION get_shipment_filter_options IS
    'Returns distinct values for filter dropdowns in the operational shipments view. '
    'No filter parameters — always returns the full universe of available values. '
    'pol_locodes[i] and pol_names[i] describe the same port (parallel arrays, ordered by locode). '
    'Same convention for pod_locodes / pod_names. '
    'vessel_names is a jsonb array of distinct shipments.vessel_name values, sorted alphabetically.';
...
VALUES ('1.7.0', 'Add p_vessel_filter to get_container_tracking_summary and get_shipment_kpis; add vessel_names output to get_shipment_filter_options')
