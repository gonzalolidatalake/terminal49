-- Shipment Operational RPCs — v1.6.0
--
-- Changes:
--   1. get_container_tracking_summary — rewritten to paginate over SHIPMENTS
--      (not containers). p_limit now means "N shipments per page". Adds
--      total_shipment_count to the return type (via COUNT(*) OVER()).
--      Breaking change: callers must update expected row counts per page.
--
--   2. get_shipment_kpis (NEW) — aggregate KPI cards for the operational view.
--      Same filters as #1 but no LIMIT/OFFSET; always scans the full filtered set.
--
--   3. get_shipment_filter_options (NEW) — distinct values for filter dropdowns.
--      No filter params; always returns the full universe.
--      Returns parallel arrays: pol_locodes[i] / pol_names[i] describe the same port.
--
-- No new indexes required — existing indexes cover all new access patterns:
--   idx_containers_shipment_id         btree(shipment_id)
--   idx_containers_shipment_status     btree(shipment_id, current_status)
--   idx_containers_number_trgm         GIN trigram(number)
--   teu_from_equipment_type()          already exists in this project

-- ============================================================================
-- RPC 1 — get_container_tracking_summary (rewritten, shipment-level pagination)
-- ============================================================================

-- Must DROP before CREATE because the return type changes (new total_shipment_count column).
...
BEGIN
    ...
    -- p_limit now means "N shipments per page" (BREAKING CHANGE from v1.5.0).
    -- Uses:
...
-- ============================================================================
-- RPC 3 — get_shipment_filter_options (NEW)
-- ============================================================================

CREATE OR REPLACE FUNCTION get_shipment_filter_options()
RETURNS TABLE (
    statuses     text[],
    scacs        text[],
    pol_locodes  text[],
...
VALUES ('1.6.0', 'Shipment-level pagination in tracking summary (total_shipment_count); add get_shipment_kpis and get_shipment_filter_options RPCs')
